...
with st.sidebar:
    ...
    st.markdown("👤 **Asesor:** `ASESOR.B2B@SOMOSTELSER.COM`")
...
# FORMULARIO ÚNICO (Clave única para evitar errores)
with st.form(key="form_ventas_final", clear_on_submit=True):
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        ...
        st.info(f"💰 **Total a Pagar: ${valor:,.0f} COP** (Dcto: {dcto}%)")
